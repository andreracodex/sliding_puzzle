...
# Checks for possible moves for search function
    def possibleMoves(self, chosenNode):
        validMoves = []
        gap_index = chosenNode.index('0')  # Assuming '0' represents the empty tile (gap)
        row, col = gap_index // 4, gap_index % 4

        # Check possible moves: left, up, right, down
        if col > 0:
            validMoves.append(gap_index - 1)  # Move left
        if row > 0:
            validMoves.append(gap_index - 4)  # Move up
        if col < 3:
            validMoves.append(gap_index + 1)  # Move right
        if row < 3:
            validMoves.append(gap_index + 4)  # Move down

        return validMoves, gap_index
    ...
    def solveBest(self):
        ...
        while not open_nodes.empty():
            ...
            for move in valid_moves:
                ...
                if child_state is not None and child_state not in closed_nodes:
                    child_node = Node(
                        state=child_state,
                        parent=current_node,
                        g=current_node.g + 1,
...
#A Star Solutions Solution
    ...
    def swapTiles(self, state, i, j):
        ...
        if 0 <= i < len(state_list) and 0 <= j < len(state_list):
